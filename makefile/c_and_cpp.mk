...
ifndef C_SRCS
    ...
    export C_SRCS := $(sort $(shell \
        ${MAKE} ${GOAL} ${GOALS} C_SRCS=.c CXX_SRCS=$(if ${CXX_SRCS},'${CXX_SRCS}',.cpp) --dry-run --always-make \
        | grep '^${CC} ' | grep " -o [\"']\?[^ ]\+\.o" \
        | sed "s/.*[ ]\+[\"']\?\([^ ]\+\.c\)[\"']\?[ ]*.*/\1/"))
endif
...
ifndef CXX_SRCS
    ...
    export CXX_SRCS := $(sort $(shell \
        ${MAKE} ${GOAL} ${GOALS} C_SRCS=$(if ${C_SRCS},'${C_SRCS}',.c) CXX_SRCS=.cpp --dry-run --always-make \
        | grep '^${CXX} ' | grep " -o [\"']\?[^ ]\+\.o" \
        | sed "s/.*[ ]\+[\"']\?\([^ ]\+\.\(cc\|cpp\|cxx\)\)[\"']\?[ ]*.*/\1/"))
endif
...
#   01. Remove the residual "_" target generated by accident
#       while guessing C_SRCS and/or CXX_SRCS.
#
